...
// Définition de nos modules d'application
window.AppModules = {
  UI: {
    init() {
    ...
    createVisualizers() {
      ...
      try {
        ...
        if (spectrogramCanvas) {
          this.visualizers.spectrogram = new SpectrogramVisualizer(
            "spectrogram-canvas"
          );
        } else {
        ...
        if (!entropyCanvas) {
          try {
            const entropyContainer = document.querySelector(
            ...
            if (entropyContainer) {
              ...
              this.visualizers.entropyMeter = new EntropyMeter(
                "entropy-visualizer"
              );
            } else {
            ...
            console.error(
              "Erreur lors de la création du canvas d'entropie:",
              error
            );
          }
        } else {
        ...
        console.error(
          "Erreur lors de l'initialisation des visualiseurs:",
          error
        );
      }
    },
    ...
    updateEntropyDisplay(entropyBits) {
      ...
      if (this.elements.entropyValue) {
        this.elements.entropyValue.textContent = `${entropyBits.toFixed(
          2
        )} bits`;
      }
      ...
      if (entropyBits >= targetEntropy) {
        ...
        this.updateStatus(
          "Entropie suffisante collectée pour la génération de clés"
        );
      } else {
        if (this.elements.generateKeyBtn) {
    ...
    updateSourceStatus(sourceName, status) {
      if (this.elements.sourceItems[sourceName]) {
        const statusElement =
          this.elements.sourceItems[sourceName].querySelector(".source-status");
        if (statusElement) {
  ...
  Audio: {
    ...
    async init() {
      ...
      try {
        ...
        if (navigator.permissions && navigator.permissions.query) {
          try {
            ...
            if (permissionStatus.state === "granted") {
              ...
              AppModules.UI.updateStatus(
                "Veuillez autoriser l'accès au microphone pour l'enregistrement"
              );
            } else if (permissionStatus.state === "denied") {
              console.error("Permission du microphone refusée");
              AppModules.UI.updateStatus(
                "L'accès au microphone est requis pour collecter l'entropie audio"
              );
        ...
        AppModules.UI.updateStatus(
          "Erreur d'initialisation audio. Le navigateur pourrait ne pas prendre en charge l'API Web Audio."
        );
        return this;
      }
    },
    ...
    calculateRMS(audioBuffer) {
      // Gardez l'original si utilisé ailleurs
    ...
    setupAudioProcessing(stream) {
      try {
        ...
        console.error(
          "Erreur lors de la configuration du traitement audio:",
          error
        );
    ...
    async processAudioFeatures(audioBlob) {
      try {
        ...
        AppModules.UI.updateStatus(
          "Caractéristiques audio extraites. Entropie calculée."
        );

        // Fournir les caractéristiques au collecteur d'entropie
        AppModules.Entropy.addEntropySource(
          "audio",
          features,
          this.estimateAudioEntropy(features)
        );
      } catch (error) {
        console.error("Erreur lors du traitement audio:", error);
        AppModules.UI.updateStatus(
          "Erreur lors de l'extraction des caractéristiques audio"
        );
      }
    },
    ...
    calculateEnergyDistribution(audioBuffer) {
      ...
      for (let i = 0; i < channelData.length; i += fftSize / 2) {
        ...
        const { magnitudes, frequencies } = this.calculateFFT(
          windowed,
          sampleRate
        );
    ...
    calculateMFCC(audioBuffer) {
      ...
      for (let i = 0; i < channelData.length; i += fftSize * frameStep) {
        ...
        for (let j = 0; j < numCoefficients; j++) {
          frameCoeffs[j] =
            rms * Math.sin((j * Math.PI) / (numCoefficients - 1)) +
    ...
    calculatePerceptualFingerprint(audioBuffer) {
      const channelData = audioBuffer.getChannelData(0);
      const frameSize = 2048; // Réduit de moitié
      const fingerprint = [];
    ...
    findSpectralPeaks(magnitudes, numPeaks) {
      ...
      for (let i = 0; i < numPeaks; i++) {
        ...
        if (maxIndex !== -1 && tempMags[maxIndex] > 0) {
          ...
          for (
            let j = Math.max(0, maxIndex - 3);
            j <= Math.min(tempMags.length - 1, maxIndex + 3);
            j++
          ) {
            tempMags[j] = 0;
          }
    ...
    estimateAudioEntropy(features) {
      ...
      if (
        ...
        features.energyDistribution.length > 0
      ) {
      ...
      if (
        features.perceptualFingerprint &&
        features.perceptualFingerprint.length > 0
      ) {
  ...
  Entropy: {
    init() {
      ...
      // Initialisation du module Entropy
      return this;
    },
    ...
    setupEntropyCollectors() {
      ...
      this.setupSystemEntropyCollector();
    },
    ...
    addEntropySource(sourceName, data, entropyEstimate = 0) {
      ...
      if (entropyEstimate > 0) {
        ...
        this.entropyEstimates[sourceName] = this.estimateSourceEntropy(
          sourceName,
          data
        );
      }
    ...
    setupMouseEntropyCollector() {
      ...
      const mouseListener = (event) => {
        ...
        if (
          mouseData.positions.length > 0 &&
          mouseData.positions.length % 50 === 0
        ) {
    ...
    setupTimingEntropyCollector() {
      ...
      const collectTimingSample = () => {
        ...
        if (timingData.samples.length > 1) {
          ...
          const diff = Math.abs(
            timingData.samples[lastIndex] - timingData.samples[lastIndex - 1]
          );
    ...
    estimateNetworkEntropy(networkData) {
      if (
        !networkData ||
        !networkData.latencies ||
        networkData.latencies.length === 0
      ) {
        return 0;
      }
    ...
    setupSensorEntropyCollector() {
      ...
      if ("AmbientLightSensor" in window) {
        try {
          ...
          console.error(
            "Erreur lors de l'initialisation du capteur de lumière:",
            error
          );
        }
      }
    ...
    async generateSeed() {
      console.log(
        "Génération de graine cryptographique à partir de l'entropie collectée"
      );
  ...
  Crypto: {
    init() {
      ...
      // Initialisation du module Cryptographie
      return this;
    },
    ...
    async init() {
      ...
      try {
        ...
        console.error(
          "Erreur lors de l'initialisation du simulateur Dilithium:",
          error
        );
        throw error;
      }
    ...
    async generateKeyPair() {
      ...
      if (
        AppModules.Entropy.currentEntropyBits <
        AppModules.Entropy.targetEntropyBits
      ) {
        AppModules.UI.updateStatus(
          `Entropie insuffisante: ${AppModules.Entropy.currentEntropyBits}/${AppModules.Entropy.targetEntropyBits} bits`
        );
        return;
      }

      AppModules.UI.updateStatus(
        "Génération de la paire de clés post-quantique..."
      );
      ...
      try {
        ...
        // Vérifier que la graine est valide (non vide et de longueur suffisante)
        if (!seed || seed.length === 0) {
          throw new Error("Graine cryptographique invalide ou vide.");
        }

        // Générer la paire de clés Dilithium
        this.keyPair = await this.dilithiumSimulator.generateKeyPair(seed);

        // Vérifier que la paire de clés est valide
        if (
          !this.keyPair ||
          !this.keyPair.publicKey ||
          !this.keyPair.privateKey
        ) {
          throw new Error("Échec de la génération de la paire de clés.");
        }

        // Afficher les clés dans l'interface utilisateur
        AppModules.UI.displayKey(this.keyPair);
        AppModules.UI.updateStatus("Paire de clés générée avec succès");

        // Retourner la paire de clés générée
        return this.keyPair;
      } catch (error) {
        console.error(
  ...
  async init() {
    ...
    try {
      ...
      try {
        ...
        AppModules.UI.updateStatus(
          "Erreur d'initialisation audio. Certaines fonctionnalités pourraient être limitées."
        );
      }

      try {
        await AppModules.Entropy.init();
        console.log("Module Entropy initialisé");
      } catch (entropyError) {
        console.error(
          "Erreur lors de l'initialisation du module d'entropie:",
          entropyError
        );
        AppModules.UI.updateStatus(
          "Erreur d'initialisation du collecteur d'entropie."
        );
      }

      try {
        await AppModules.Crypto.init();
        console.log("Module Crypto initialisé");
      } catch (cryptoError) {
        console.error(
          "Erreur lors de l'initialisation cryptographique:",
          cryptoError
        );
      ...
      AppModules.UI.updateStatus(
        "Prêt. Cliquez sur \"Démarrer l'enregistrement\" pour collecter de l'entropie audio."
      );

      // Initialiser l'entropie système dès le départ
      try {
        AppModules.Entropy.setupSystemEntropyCollector();
        console.log("Collecteur d'entropie système initialisé");
      } catch (systemEntropyError) {
        console.error(
          "Erreur lors de l'initialisation de l'entropie système:",
          systemEntropyError
        );
      }

      // Déclencher un événement personnalisé pour signaler que l'initialisation est terminée
      const event = new Event("appModulesInitialized");
      ...
      try {
        ...
        if (statusElement) {
          statusElement.textContent =
            "Erreur d'initialisation. Veuillez recharger la page.";
